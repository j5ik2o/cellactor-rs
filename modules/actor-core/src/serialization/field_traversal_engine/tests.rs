...
use crate::serialization::{
  AggregateSchemaBuilder, FieldPath, FieldPathDisplay, FieldPathSegment, FieldTraversalEngine, TraversalPolicy,
};
...
fn depth_first_plan_orders_by_path() {
  ...
  let order = plan.iter().map(|index| schema.fields()[index].display().as_str()).collect::<Vec<_>>();
...
fn build_schema(policy: TraversalPolicy) -> crate::serialization::AggregateSchema {
  let mut builder =
    AggregateSchemaBuilder::<RootAggregate>::new(policy, FieldPathDisplay::from_str("root").expect("display"));
  builder
