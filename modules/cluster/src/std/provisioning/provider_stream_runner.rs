...
use crate::std::provisioning::failover_controller::FailoverController;
use crate::std::provisioning::partition_manager_bridge::PartitionManagerBridge;
use crate::std::provisioning::placement_supervisor_bridge::PlacementSupervisorBridge;
...
impl ProviderStreamRunner {
  ...
  pub fn new(
    ...
    for (desc, stream) in streams {
      drivers.insert(desc.id().as_str().to_string(), ProviderStreamDriver::new(stream, hub.clone(), placement.clone(), partition.clone(), metrics.clone()));
    }
  ...
  fn ensure_active(&mut self) -> bool {
    ...
    if let Some(desc) = self.failover.select_active() {
      ...
      if let Some(prev) = self.current.replace(next_id.clone()) {
        ...
        let from = ProviderDescriptor::new(crate::core::provisioning::descriptor::ProviderId::new(prev.clone()), desc.kind().clone(), desc.priority()).id().clone();
  ...
  pub fn pump_once(&mut self) -> RunnerProgress {
    ...
    match driver.pump_once(&mut self.seq_no) {
      ...
      Ok(StreamProgress::Terminated) => {
        ...
        if self.ensure_active() {
          RunnerProgress::Switched
        } else {
          RunnerProgress::Exhausted
        }
      },
      Err(_) => {
        // エラー時は failover して次へ
