...
use fraktor_actor_rs::core::actor_prim::{Actor, ActorContextGeneric, Pid};
...
use crate::core::activation::{ActivationError, ActivationLease, ActivationRequest, LeaseId, LeaseStatus};
use crate::core::identity::{ClusterIdentity, NodeId};
use crate::core::placement::{PlacementActor, PlacementSpawner, PlacementSpawnerError};
...
impl PlacementSpawner<NoStdToolbox> for RecordingSpawner {
    fn spawn(
        &self,
        _: &ClusterIdentity,
        _: PropsGeneric<NoStdToolbox>,
    ) -> Result<Pid, PlacementSpawnerError> {
        self.responses
            .get(0)
            .cloned()
            .unwrap_or_else(|| Err(PlacementSpawnerError::SpawnFailed))
    }
}
