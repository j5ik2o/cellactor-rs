...
use super::{
  cluster_identity::ClusterIdentity, cluster_node::ClusterNode, node_id::NodeId, topology_snapshot::TopologySnapshot,
};
...
impl HashRingProvider {
  ...
  pub fn rebuild(&mut self, snapshot: &TopologySnapshot) {
    self.nodes.clear();
    self.nodes.extend(snapshot.members().iter().filter(|node| !node.is_blocked()).cloned());
