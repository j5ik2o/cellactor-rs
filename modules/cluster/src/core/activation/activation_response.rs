...
use super::{activation_error::ActivationError, lease_id::LeaseId};
...
impl ActivationResponse {
  ...
  #[must_use]
  pub const fn success(identity: ClusterIdentity, pid: Pid, lease_id: LeaseId, topology_hash: u64) -> Self {
  ...
  #[must_use]
  pub const fn error(&self) -> Option<&ActivationError> {
    self.error.as_ref()
  }
}
